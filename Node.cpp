...
void Node::calc_deriv_price(float K, float R, float pi, Node& next_up, Node& next_down){
    ...
    float a_tmp = (1/R) * (pi * next_up.a_put_price + (1-pi) * next_down.a_put_price);
    this->a_put_price = a_tmp > K-share_price ? a_tmp : K-share_price;
}
